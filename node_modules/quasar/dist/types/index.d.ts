...
export interface Cookies {
   ...
   * @returns Cookie object (like $q.cookies) for SSR/SSG usage purposes
   */
...
export interface IconSet {
   ...
   * @param ssrContent Required for SSR/SSG only
   */
  set: (
    iconSet: {
      /**
...
export interface Lang {
  ...
  props: {
     ...
     * @param value The ASCII digit string the component would display
     * @returns The localized digit string
     */
    ...
    date: {
       ...
       * @param range Number of years in the range
       * @returns The label
       */
   ...
   * @param locale BCP 47 language tag (an underscore separator is accepted too); undefined yields undefined, so the result of getLocale() can be passed directly
   * @param isoNames Language pack names (their isoName) to choose from; the entries are returned as given
   * @returns The best matching entry of the list; undefined when no entry shares the locale's language
   */
...
export interface Platform {
   ...
   * @param ssrContext SSR Context Object
   * @returns Platform object (like $q.platform) for SSR/SSG usage purposes
   */
...
/**
 * Boolean false (or undefined) to disable the directive (the observer is dropped and recreated when a handler is supplied again); Function to call when mutation occurs
 *
 ...
 *  - early:
 *    - type: boolean
 *    - description: Trigger early/immediately on user interaction; the ripple auto-cancels if the gesture turns into a scroll/pan or the pressed pointer is dragged off the element
 *  - stop:
 *    - type: boolean
 ...
 *  - type: number
 *  - default: 0
 *  - description: Fraction of the element (0 to 1) that must be visible before firing; 0 (the default) fires as soon as any part of it is visible, 1 only once it is fully visible
 *  - examples:
 *    - # v-scroll-fire:0.5
 *    - # v-scroll-fire:1
 *
...
export interface QBtnDropdownProps {
   ...
   * Default value: 150
   */
  hoverHideDelay?: number | undefined;
  /**
   ...
   */
  toggleAriaHaspopup?: string | undefined;
  /**
...
export interface QEditorProps {
   ...
   */
  dropdownHover?: boolean | undefined;
  /**
   ...
   * Default value: 0
   */
  dropdownHoverDelay?: number | undefined;
  /**
   ...
   * Default value: 150
   */
  dropdownHoverHideDelay?: number | undefined;
  /**
   * Emitted when fullscreen state changes
   * @param value Fullscreen state (showing/hidden)
   */
...
export interface QFormProps {
   ...
   */
  onValidationError?: (ref: ComponentPublicInstance) => void;
}
...
export interface QInfiniteScrollProps {
   ...
   */
  scrollTarget?: Element | string | ComponentPublicInstance | undefined;
  /**
   * Put component in disabled mode
   */
  disable?: boolean | undefined;
...
export interface QRangeProps {
   ...
   */
  leftThumbAriaLabel?: string | undefined;
  /**
   ...
   */
  rightThumbAriaLabel?: string | undefined;
  /**
...
export interface QSelectProps {
   ...
   */
  hideDialogClose?: boolean | undefined;
  /**
   * Icon name following Quasar convention; Make sure you have the icon library installed unless you are using 'img:' prefix; If 'none' (String) is used as value then no icon is rendered (but screen real estate will still be used for it)
   */
  dropdownIcon?: string | undefined;
   ...
   */
  noChipRemove?: boolean | undefined;
  /**
   ...
   */
  noOptionPrefetch?: boolean | undefined;
  /**
   ...
   * @param evt JS event object; Is undefined when the popup is not triggered by a direct user interaction (e.g. it opens as an async filter resolves or through the showPopup method)
   */
  onPopupShow?: (evt?: Event) => void;
  /**
   * Emitted when the select options menu or dialog is hidden.
   * @param evt JS event object; Is undefined when the popup is not dismissed by a direct user interaction (e.g. it closes because an option got selected or through the hidePopup method)
   */
...
export interface QSplitterProps {
   ...
   */
  separatorAriaLabel?: string | undefined;
  /**
...
export interface QTableProps {
   ...
   * CSS selector, DOM element or Vue component reference (standing for its root element) to be used as a custom scroll container instead of the auto detected one
   */
  virtualScrollTarget?: Element | string | ComponentPublicInstance | undefined;
  /**
...
export interface QTooltipProps {
   ...
   */
  cursorPosition?: boolean | undefined;
  /**
...
export interface QTree extends ComponentPublicInstance<QTreeProps> {
   ...
   * @param edge The edge to align to if the row is not visible already (by default it aligns to end if scrolling towards the end and to start otherwise); If the '-force' version is used then it always aligns
   */
  scrollTo: (
    key: any,
    edge?: "start" | "center" | "end" | "start-force" | "center-force" | "end-force",
   ...
   * @param key The key of a node
   * @returns The parent node, or 'undefined' if the node is a root one or the key is not in the nodes model
   */
  getParentNode: (key: any) => any;
  /**
   ...
   * @returns Partially ticked node objects, in the order of the nodes model
   */
  getIndeterminateNodes: () => readonly any[];
  /**
   ...
   * @param key The key of a node
   * @returns Is specified node partially ticked?
   */
   ...
   * @param key The key of a node
   * @returns The tick state of the specified node ('false' if the key is not in the nodes model)
   */
  getTickState: (key: any) => boolean | null;
  /**
...
export interface QVirtualScrollProps {
   ...
   */
  separator?: boolean | string | undefined;
  /**
   * Applies a default border to the component
   */
  bordered?: boolean | undefined;
...
export interface DialogChainObject {
   ...
   */
  onCancel: (
    callbackFn: (reason?: "cancel" | "backdrop" | "escape" | "programmatic") => void,
  ) => DialogChainObject;
  /**
   ...
   * @returns Chained Object
   */
  onDismiss: (callbackFn: (payload?: any) => void) => DialogChainObject;
  /**
...
import { LangGetClosestIsoNameMethodType } from "./api";
